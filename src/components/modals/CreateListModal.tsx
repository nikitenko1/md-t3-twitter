...
const CreateListModal = () => {
  ...
  const { mutateAsync: createList } = api.list.createList.useMutation({
    onMutate: () => {
  ...
  const handleSubmit = async () => {
    ...
    await toast.promise(
      createList({
        ...
        description: desc,
        isPrivate,
        coverPhoto: coverPhotoUrl as string,
      }),
      {
      ...
      >
        <header className="flex items-center justify-between gap-x-1 p-4 xs:gap-x-2 sm:gap-x-4">
          ...
          <button
            ...
            type="submit"
            className={`${
              name.length <= 0 ? "bg-slate-300" : null
            } rounded-full bg-black px-2 py-1 text-xs font-semibold text-white sm:text-sm md:px-3`}
          >
            Next
          </button>
        </header>
