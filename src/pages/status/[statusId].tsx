...
import { LikesWithPayloads, OptionWithPayload, TweetWithUser } from "interface";
import {
  useLikesModal,
  usePhotoView,
  usePhotoViewModal,
...
const StatusPage = () => {
  ...
  const { data: tweetDetails, isLoading } = api?.tweet.getSingleTweet.useQuery({
    tweetId: statusId,
  });
  ...
  const { data: replies } = api?.tweet.getTweetReplies.useQuery({
    tweetId: statusId,
  });
  ...
  const { handleLike, hasLiked, handleUnlike, likeLoading, unlikeLoading } =
  ...
  return (
    <Body>
      <Head>
        <title>{tweetDetails?.user.name} on UA You</title>
        ...
        <div className="space-y-2 p-4 md:space-y-4">
          ...
          {tweetDetails?.poll ? (
            <div className="flex flex-col gap-y-2">
              <VoteComponentList
                options={tweetDetails.poll.options as OptionWithPayload[]}
              />
            </div>
          ) : null}
          {tweetDetails?.image ? (
          ...
          <div
            className="flex items-center justify-evenly gap-x-4 border-b border-base-200
          px-2 pb-4 text-xl text-gray-400"
          >
            <AiOutlineComment
