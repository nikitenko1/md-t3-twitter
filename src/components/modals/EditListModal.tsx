...
const EditListModal = () => {
  ...
  const { data: listDetails } = api.list.getListDetails.useQuery({
    listId: listId as string,
  });

  const { mutateAsync: editList } = api.list.editList.useMutation({
    onMutate: () => {
  ...
  const { mutateAsync: deleteList } = api.list.deleteList.useMutation({
    onMutate: () => {
      utils.list.getListDetails.invalidate({ listId: listId as string });
      const optimisticUpdate = utils.list.getListDetails.getData();
      if (optimisticUpdate) {
  ...
  return (
    <Modal>
      <div
        ref={modalRef}
        className="relative mx-auto flex h-[550px] max-w-2xl gap-x-2 overflow-x-hidden
        overflow-y-scroll rounded-2xl bg-base-100"
      >
