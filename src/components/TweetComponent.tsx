...
const TweetComponent = ({
  ...
  const { data: alreadyRetweeted } = api.tweet.userAlreadyRetweet.useQuery(
    {
  ...
  const { data: alreadyLiked } = api.like.userLikeTweet.useQuery({
    tweetId: tweet.id as string,
  });
  const { setTweetId } = useTweetId();

  const { handleLike, handleUnlike, hasLiked, likeLoading, unlikeLoading } =
    useLikeTweet(tweet);
